...
export function assertAuthBootConfig(env: NodeJS.ProcessEnv = process.env): void {
  ...
  if (isProduction) {
    requireEnv('AUTH_ACCESS_TOKEN_SECRET', { production: true, minLength: 32, disallowDefaults: true })
    requireEnv('AUTH_PASSWORD_PEPPER', { production: true, minLength: 32, disallowDefaults: true })
